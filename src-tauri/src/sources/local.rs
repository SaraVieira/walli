...
impl WallpaperSource for Local {
    ...
    async fn fetch(&self, ctx: &FetchContext) -> AppResult<FetchedImage> {
        ...
        if !root.exists() {
            return Err(AppError::Invalid("Local folder does not exist".into()));
        }
        let files = scan(root);
        tracing::debug!(folder = %folder, count = files.len(), "local folder scanned");
        if files.is_empty() {
            tracing::warn!(folder = %folder, "local folder has no images");
        }
        let pick = files
