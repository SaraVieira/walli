...
pub fn upsert_wallpaper(pool: &Pool, w: &Wallpaper) -> AppResult<i64> {
    ...
    let id: i64 = conn.query_row(
        ...
        params![w.source, w.source_id], |r| r.get(0))?;
    Ok(id)
}
...
pub fn record_history(pool: &Pool, wallpaper_id: i64, set_at: i64, display_id: Option<&str>) -> AppResult<i64> {
    let conn = pool.lock().unwrap();
    conn.execute(
...
pub fn list_history(pool: &Pool, limit: u32, offset: u32, favorites_only: bool) -> AppResult<Vec<HistoryEntry>> {
    let conn = pool.lock().unwrap();
    ...
    let rows = stmt.query_map(params![limit, offset], |r| {
        Ok(HistoryEntry {
            history_id: r.get(0)?, set_at: r.get(1)?, display_id: r.get(2)?,
            wallpaper: Wallpaper {
                id: r.get(3)?, source: r.get(4)?, source_id: r.get(5)?,
                photographer: r.get(6)?, source_url: r.get(7)?, file_path: r.get(8)?,
                is_local: r.get::<_, i64>(9)? != 0,
                width: r.get(10)?, height: r.get(11)?, fetched_at: r.get(12)?,
            },
...
pub fn toggle_favorite(pool: &Pool, wallpaper_id: i64, now: i64) -> AppResult<bool> {
    let conn = pool.lock().unwrap();
    let exists: bool = conn.query_row(
        "SELECT EXISTS(SELECT 1 FROM favorites WHERE wallpaper_id = ?)",
        params![wallpaper_id], |r| r.get::<_, i64>(0).map(|v| v != 0))?;
    if exists {
        conn.execute("DELETE FROM favorites WHERE wallpaper_id = ?", params![wallpaper_id])?;
        Ok(false)
    } else {
        conn.execute("INSERT INTO favorites (wallpaper_id, favorited_at) VALUES (?, ?)",
            params![wallpaper_id, now])?;
        Ok(true)
    }
}
...
pub fn list_collections(pool: &Pool) -> AppResult<Vec<Collection>> {
    ...
    let cols: Vec<(i64, String)> = stmt.query_map([], |r| Ok((r.get(0)?, r.get(1)?)))?
        .collect::<rusqlite::Result<_>>()?;
    let mut out = Vec::new();
    for (id, name) in cols {
        let mut tag_stmt = conn.prepare("SELECT tag FROM collection_tags WHERE collection_id = ? ORDER BY tag")?;
        let tags: Vec<String> = tag_stmt.query_map(params![id], |r| r.get::<_, String>(0))?
            .collect::<rusqlite::Result<_>>()?;
...
pub fn create_collection(pool: &Pool, name: &str, tags: &[String], now: i64) -> AppResult<Collection> {
    let mut conn = pool.lock().unwrap();
    let tx = conn.transaction()?;
    tx.execute("INSERT INTO collections (name, created_at) VALUES (?, ?)", params![name, now])?;
    let id = tx.last_insert_rowid();
    for tag in tags {
        tx.execute("INSERT INTO collection_tags (collection_id, tag) VALUES (?, ?)", params![id, tag])?;
    }
    tx.commit()?;
    Ok(Collection { id, name: name.to_string(), tags: tags.to_vec() })
}

pub fn update_collection(pool: &Pool, id: i64, name: &str, tags: &[String]) -> AppResult<Collection> {
    let mut conn = pool.lock().unwrap();
    let tx = conn.transaction()?;
    tx.execute("UPDATE collections SET name = ? WHERE id = ?", params![name, id])?;
    tx.execute("DELETE FROM collection_tags WHERE collection_id = ?", params![id])?;
    for tag in tags {
        tx.execute("INSERT INTO collection_tags (collection_id, tag) VALUES (?, ?)", params![id, tag])?;
    }
    tx.commit()?;
...
pub fn delete_collection(pool: &Pool, id: i64) -> AppResult<()> {
    pool.lock().unwrap().execute("DELETE FROM collections WHERE id = ?", params![id])?;
    Ok(())
}
