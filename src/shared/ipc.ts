...
import type { AppState, Collection, HistoryEntry, Settings, SourceKind, Wallpaper } from "./types";

export const ipc = {
  getState: () => invoke<AppState>("get_state"),
  nextNow: () => invoke<void>("next_now"),
  setPaused: (paused: boolean) => invoke<void>("set_paused", { paused }),
  toggleFavorite: (wallpaperId: number) => invoke<void>("toggle_favorite", { wallpaperId }),
  setWallpaperFromHistory: (wallpaperId: number) => invoke<void>("set_wallpaper_from_history", { wallpaperId }),
  ...
  createCollection: (name: string, tags: string[]) => invoke<Collection>("create_collection", { name, tags }),
  ...
  setActiveCollection: (id: number) => invoke<void>("set_active_collection", { id }),
  getSettings: () => invoke<Settings>("get_settings"),
  updateSettings: (patch: Partial<Settings>) => invoke<Settings>("update_settings", { patch }),
  setApiKey: (source: SourceKind, key: string) => invoke<void>("set_api_key", { source, key }),
  clearApiKey: (source: SourceKind) => invoke<void>("clear_api_key", { source }),
  pickLocalFolder: () => invoke<string | null>("pick_local_folder"),
  setLoginAtStartup: (enabled: boolean) => invoke<void>("set_login_at_startup", { enabled }),
};

export function onWallpaperChanged(handler: (w: Wallpaper) => void): Promise<UnlistenFn> {
